...
public class CMDeleteLinkElementEdit extends CMComponentAwareEdit{

	private CMCompoundEdit m_ce;
	private CMFrameView m_frame;
	private boolean isPanelTestDataSelected;

	public CMDeleteLinkElementEdit(CMUndoMediator.FocusInfo p_FocusInfo, CMFrameView p_CMFrameView, UndoableEdit p_ce) {
		super(p_FocusInfo, p_CMFrameView);
		this.m_ce = (CMCompoundEdit) p_ce;
		this.m_frame = p_CMFrameView;
		isPanelTestDataSelected = m_frame.isIsPanelTestDataSelected();
	}
	...
	public void redo() throws CannotRedoException {
        ...
        m_frame.getM_CMUndoMediator().setM_NumberOfRedoableEditObjects(m);
   		if(isPanelTestDataSelected)
   			m_frame.getPanelTestDataView().getM_CMGridTDStructure().update(CMIndexTDStructureUpdate.getInstance().getIndex());
   		else
   			m_frame.getGridTDStructure().update(CMIndexTDStructureUpdate.getInstance().getIndex());
