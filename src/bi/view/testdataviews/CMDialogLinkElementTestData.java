...
public class CMDialogLinkElementTestData extends JCustomDialog{
/**
	 * 24/11/2006
	 * svonborries
	 */
	...
	private TDStructure m_TDStructureStructure;
	private TestDataCombinations m_TestDataCombination;
	private TestData m_TestData;
	private JPanel jPanelGlobal = new JPanel();
	...
	private JList jListGlobal = new JList();
	...
	public CMDialogLinkElementTestData (){
...
//		HCanedo_17112005_end
		...
		this.m_TestDataCombination = this.m_TDStructure.getTestDataCombination();
		m_Frame.getGridTDStructure();
		try {
			initGUI();
		} catch (Exception e) {
			e.printStackTrace();
	...
	void initGUI() throws Exception{
		...
		setBounds(new Rectangle(0, 0, 500, 354));
		setResizable(false);
		getRootPane().setDefaultButton(jButtonOk);

//panel jPanelGlobal
		this.jPanelGlobal.setBounds(new Rectangle(26,15,135,181));
		this.jPanelGlobal.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(),
	            CMMessages.getString("TESTDATA_LINKELEMENT_GLOBAL"), TitledBorder.LEADING, TitledBorder.TOP,
	            new Font("SansSerif", 0, 11), new Color(60, 60, 60)));
		this.jPanelGlobal.setLayout(new BorderLayout());
		this.jPanelGlobal.add(jScrollPaneGlobal,BorderLayout.CENTER);
		this.jScrollPaneGlobal.getViewport().add(jListGlobal);
		this.jListGlobal.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);

		this.jListGlobal.addListSelectionListener(new ListSelectionListener(){

			public void valueChanged(ListSelectionEvent arg0) {
				jListGlobalValueChange(arg0);

			}});

//panel jPanelStructure
		this.jPanelStructure.setBounds(new Rectangle(178, 15, 135, 181));
...
//panel jPanelField
		this.jPanelField.setBounds(new Rectangle(328, 15, 135, 181));
...
//textarea jTextAreaField
        ...
        this.fillGlobalList();
        this.jListGlobal.setSelectedIndex(0);

		this.fillStructureList(this.jListGlobal.getSelectedIndex());
    ...
    private void fillGlobalList(){
    	List l_Global = new Vector();
    	l_Global.add(CMMessages.getString("TESTDATA_LINKELEMENT_GLOBAL"));

    	for (Iterator iter = this.m_TestDataCombination.getM_TestDatas().iterator(); iter.hasNext();) {
    		l_Global.add(((TestData)iter.next()).getName());
		}
    	 this.jListGlobal.setListData((Vector)l_Global);
    }

    private void fillStructureList(int p_GlobalIndex){
    	List l_Structure = new Vector();
    	...
    	if(p_GlobalIndex == 0){
    		...
    		m_TestData = (TestData) this.m_TestDataCombination.getM_TestDatas().get(p_GlobalIndex-1);
    ...
    private void fillFieldList(int p_StructureIndex) {
    	List l_TypeData = new Vector();
    	if (p_StructureIndex == -1 ){
    		p_StructureIndex = 0;
    		}
    	if(this.jListGlobal.getSelectedIndex() == 0){
    ...
    public void jButtonOKActionPerformed(ActionEvent e) {
    	...
    	if (this.m_TypeData!=null){
    		ITypeData observer= null;//new TypeData();
			if(m_TypeDataFotInsertField!= null){
				observer= m_TypeDataFotInsertField;
			}
			else{
			...
			if(!existCircularReferences(observer, m_TypeData)){
				...
				if(!isOpenInDialogFormulas()){
					...
					//TDStructureManager.getTypeDataReferenceForGlobalStructure(m_Frame, observer, getM_TypeData(), m_StringFormulaField, true);
				...
				//	TDStructureManager.getTypeDataReferenceForGlobalStructure(m_Frame, observer, getM_TypeData(), m_StringFormulaField, false);
	...
	public void jButtonCancelActionPerformed(ActionEvent arg0) {
		...
		this.m_TDStructureStructure = null;
		this.m_TestData = null;
		this.m_TestDataCombination = null;
		this.m_StringFormulaField = "";
        cancel();
	}

	public void jListGlobalValueChange(ListSelectionEvent arg0){
		this.fillStructureList(this.jListGlobal.getSelectedIndex());
		this.jListStructure.setSelectedIndex(0);
	}
	...
	public void jListFieldValueChanged(ListSelectionEvent arg0) {
		this.fillLabelValue(this.jListField.getSelectedIndex(),this.jListStructure.getSelectedIndex(),this.jListGlobal.getSelectedIndex());
	...
	protected List getOrder() {
		...
		focusOrder.add(this.jListGlobal);
	...
	public TestData getM_TestData() {
		return m_TestData;
	}


	public void setM_TestData(TestData testData) {
		m_TestData = testData;
	}
	...
	public void setTypeDataForInsertField(ITypeData typeDataForInsertField) {
		m_TypeDataFotInsertField=typeDataForInsertField;

	}
	private ITypeData m_TypeDataFotInsertField;
}
//svonborries_08112005_end