...
public class CMGenerateTestDataCombinationsFromFaultyTestCasesAction extends
	...
	public CMGenerateTestDataCombinationsFromFaultyTestCasesAction(){
		super(CMMessages.getString("TESTDATA_GENERATE_FAULTY_TESTDATA"));
		putValue(Action.SHORT_DESCRIPTION, CMMessages.getString("TESTDATA_GENERATE_FAULTY_TESTDATA"));
	    putValue(Action.SMALL_ICON, CMAction.TESTDATA_GENERATE_TESTDATA_FROM_ALL_FAULTY.getIcon());
	    putValue(Action.MNEMONIC_KEY,(int)CMMessages.getString("GENERATE_TEST_DATA_COMBINATIONS_FROM_THE_FAULTY_TEST_CASE_MNEMONIC").charAt(0));
	    putValue(Action.ACCELERATOR_KEY, KeyStroke.getKeyStroke(java.awt.event.KeyEvent.VK_4, Event.CTRL_MASK));
	}
	...
	public void actionPerformed(ActionEvent e) {
	    ...
	    final TDStructure tdStructure = this.m_frame.getTreeWorkspaceView().getSelectedTDStructure();
	    ...
	    if(this.m_gridTDStructure.existTestCase())
	        ...
	        if (tdStructure.getTestDataCombination().getM_TestDatas().size() > 0 ||
	            ...
	            confirmation = JOptionPane.showConfirmDialog(this.m_frame,
	                ...
	                CMMessages.getString("TESTDATA_TITLE_MESSAGE_DELETE_FAULTY_TESTDATA"),
	        ...
	        if (confirmation == JOptionPane.YES_OPTION) {
	            ...
	            this.m_gridTDStructure.GenerateTDStructureForFaultyTestCase(createStructure,ce, intervalAmount);
	        	...
	        	/*this.m_frame.getM_CMUndoMediator().generateFaultyDeletedTDStructure(this.m_gridTDStructure,
	        			clontdStructure,index,this.m_frame.getm_TabbedPaneView(),
	        ...
	        else if(confirmation ==JOptionPane.NO_OPTION)
	        {
				...
				createStructure=this.m_gridTDStructure.insertDataTDStructureView(ce);
	            //index =this.m_gridTDStructure.getTDStructure().getM_StructureTestData().size()-1;
	            //this.m_gridTDStructure.GenerateTDStructureForPositiveTestCase(createStructure,ce);
				this.m_gridTDStructure.GenerateTDStructureForFaultyTestCase(createStructure,ce, intervalAmount);
	            ce.addEdit(CMViewEditFactory.INSTANCE.createChangeIndexInCMIndexTDStructureUpdate(CMIndexTDStructureUpdate.getInstance(),0));
	            CMIndexTDStructureUpdate.getInstance().setindex(0);
	            ce.addEdit(CMViewEditFactory.INSTANCE.createChangeIndexTestDataInCMIndexTDStructureUpdate(CMIndexTDStructureUpdate.getInstance(),0));
				...
				if(ce.hasEdits())
		        	CMUndoMediator.getInstance().doEdit(ce);
	            /*this.m_frame.getM_CMUndoMediator().generateFaultyTDStructure(this.m_gridTDStructure,index,
