 ...
 *
 */
public class CMWorkFlowPositiveTestCaseGenerator extends CMAbstractTestCaseGenerator
		implements CMTestCaseGenerator {

	/**
	 */
	public CMWorkFlowPositiveTestCaseGenerator() {
		super();
	}
	 ...
	 */
	public CMWorkFlowPositiveTestCaseGenerator(Structure p_structure, List<Dependency> p_dependencies) {
	...
	public UndoableEdit generate() throws CMTestCaseGenerationException {
	    ...
	    TestCaseGroup testCaseGroup = (TestCaseGroup) structure.getM_TestCaseGroups().get(BusinessRules.TEST_CASE_GROUP_POSITION_OF_POSITIVES);
	    ...
	    List<TestCase> testCases = generateWorkFlowTestCases(ce, CMStateBean.STATE_POSITIVE, testCaseGroup);
	    ...
	    if( overallSizeOfTheSameTypeOfExistingTestCases <= CMApplication.frame.getTreeWorkspaceView().getM_Session2().getM_ApplicationSetting().getM_MaxNumOfPositiveTestCases()) {
	      ...
	      ce.addEdit(insertTestCasesAt(BusinessRules.TEST_CASE_GROUP_POSITION_OF_POSITIVES,testCaseGroup, dependencies, testCases, structure));
	    }
	    else
...
public String toString() {
	...
	return CMMessages.getString("LABEL_POSITIVE_WORKFLOW_TESTCASES"); //$NON-NLS-1$
}
}
