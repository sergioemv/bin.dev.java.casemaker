 ...
 *
 */
public class CMWorkFlowFaultyTestCaseGenerator extends CMAbstractTestCaseGenerator
		implements CMTestCaseGenerator {

	/**
	 */
	public CMWorkFlowFaultyTestCaseGenerator() {
		super();
	}
	 ...
	 */
	public CMWorkFlowFaultyTestCaseGenerator(Structure p_structure, List<Dependency> p_dependencies) {
	...
	public UndoableEdit generate() throws CMTestCaseGenerationException {
	    ...
	    TestCaseGroup testCaseGroup = (TestCaseGroup) structure.getM_TestCaseGroups().get(BusinessRules.TEST_CASE_GROUP_POSITION_OF_FAULTIES);
	    ...
	    List<TestCase> testCases = generateWorkFlowTestCases(ce, CMStateBean.STATE_FAULTY, testCaseGroup);
	    ...
	    if( overallSizeOfTheSameTypeOfExistingTestCases <= CMApplication.frame.getTreeWorkspaceView().getM_Session2().getM_ApplicationSetting().getM_MaxNumOfPositiveTestCases()) {
	      ...
	      ce.addEdit(insertTestCasesAt(BusinessRules.TEST_CASE_GROUP_POSITION_OF_FAULTIES,testCaseGroup, dependencies, testCases, structure));
	    }
	    else
...
public String toString() {
	...
	return CMMessages.getString("LABEL_FAULTY_WORKFLOW_TESTCASES"); //$NON-NLS-1$
}
}
